...
import org.iatoki.judgels.michael.operation.html.listCreateOperationsView;
...
public final class OperationController extends AbstractJudgelsController {
    ...
    public Result postRunOperation(long operationId) throws OperationNotFoundException {
        ...
        if (formHasErrors(operationRunForm)) {
            return showRunOperation(operation, operationAdapter.getRunHtml(operationRunForm, routes.OperationController.postRunOperation(operation.getId()), Messages.get("operation.run"), machineService.getAllMachines(), applicationService.getAllApplications()));
        }
        ...
        if (!operationAdapter.runOperation(operationRunForm, machineService, machineAccessService, applicationService, applicationVersionService, operation.getConf())) {
            return redirect(routes.OperationController.operationFail(operation.getId()));
        }

        return redirect(routes.OperationController.operationSuccess(operation.getId()));
    }
